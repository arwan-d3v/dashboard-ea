...
#property description "SLOWLY but SURELY"
...
input double         BasketProfitPips     = 44.75;      // Basket Profit (All Positions)
...
input double         MaxDailyProfitUSD    = 100000.0;   // Max Daily Profit ($)
...
void OnTick()
{
   ...
   // Logic: Jika ada posisi terbuka (totalPos > 0), abaikan time filter & daily target (selesaikan averaging dulu)
   if(g_DailyTargetHit) {
...
void CheckAndOpenGrid(double ask, double bid)
{
   ...
   if(g_PrimaryDir == "BUY") {
      // Floating loss for BUY means price drops
      ...
      // Floating loss for SELL means price rises
      nextGridPrice = NormalizeDouble(g_CenterPrice + ((g_CurrentLayer + 1) * gridDistancePrice), _Digits);
   ...
   if(triggerGrid) {
      ...
      // Open BOTH (Averaging & Scalping)
      if(g_PrimaryDir == "BUY") {
         trade.Buy(lotSize, _Symbol, ask, 0, 0, "AVG_B"); // Averaging
         trade.Sell(lotSize, _Symbol, bid, 0, 0, "SCL_S"); // Scalping
      } else {
         trade.Sell(lotSize, _Symbol, bid, 0, 0, "AVG_S"); // Averaging
         trade.Buy(lotSize, _Symbol, ask, 0, 0, "SCL_B"); // Scalping
      }
...
//+------------------------------------------------------------------+
//| Hidden Take Profit Logic (REVISED)                               |
//+------------------------------------------------------------------+
void CheckHiddenTPs(double totalFloating, double totalLots)
{
   ...
   // 1. Jika HANYA ada posisi Inisial (Layer 0), abaikan Basket TP dan kejar Trend Follow TP
   if(g_CurrentLayer == 0) {
   ...
   // 2. Jika SUDAH ada Averaging Grid (Layer > 0), abaikan Trend Follow dan gunakan Basket TP
   else {
   ...
   // 3. Scalping TP (Individual close for SCL positions)
...
//+------------------------------------------------------------------+
//| Utility: Close All Magic Positions                               |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Utility: Recover State on Restart                                |
//+------------------------------------------------------------------+
void RecoverState()
{
   for(int i=PositionsTotal()-1; i>=0; i--) {
   ...
   // Calculate max layer based on open positions
   int maxAvg = 0;
   for(int i=PositionsTotal()-1; i>=0; i--) {
      if(pos.SelectByIndex(i) && pos.Magic() == MagicNumber && pos.Symbol() == _Symbol) {
         string c = pos.Comment();
...
//+------------------------------------------------------------------+
//| Utility: Check Time Filter (WITA GMT+8)                          |
//+------------------------------------------------------------------+
bool CheckTimeFilter(MqlDateTime &dtLocal)
{
   ...
   if (startMins < stopMins) {
      ...
      return (currentMins >= startMins || currentMins < stopMins); // Crosses midnight
   }
}

//+------------------------------------------------------------------+
//| Utility: Get Today's Closed Profit (Local Time Base)             |
//+------------------------------------------------------------------+
double GetClosedProfitTodayLocal()
{
   // Find server time midnight based on local time difference
...
void UpdateDashboard(string status, int bCount, double bLot, int sCount, double sLot, double floatUSD, double dailyUSD)
{
   ...
   ObjectSetString(0, "DASH_MAGIC",  OBJPROP_TEXT, "Magic: " + IntegerToString(MagicNumber) + " | v1.01");
}
//+------------------------------------------------------------------+
